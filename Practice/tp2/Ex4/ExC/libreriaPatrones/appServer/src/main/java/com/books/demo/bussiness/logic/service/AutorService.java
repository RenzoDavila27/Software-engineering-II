...
public class AutorService extends BaseService<Autor> {
    ...
    @Transactional
    public Autor clonarAutor(Long id) throws ErrorServiceException {
        Autor original = obtenerEntidad(id);
        Autor clon = original.clonar();
        return crearAutor(clon);
    }
    ...
    @Transactional(readOnly = true)
    public List<Long> buscarIdsPorNombreSimilar(String criterio) throws ErrorServiceException {
        if (!StringUtils.hasText(criterio)) {
            return List.of();
        }
        return autorRepository.buscarPorNombreSimilar(criterio.trim()).stream()
                .map(Autor::getId)
