...
public class FacturaService {
    ...
    public void generarFacturaPDF(Factura factura, OutputStream outputStream) throws IOException {
        PdfWriter writer = new PdfWriter(outputStream);
        PdfDocument pdf = new PdfDocument(writer);
        Document document = new Document(pdf);

        document.add(new Paragraph("GIMNASIO SPORT")
                .setBold()
                .setFontSize(20)
                .setTextAlignment(TextAlignment.CENTER));

        document.add(new Paragraph("Direccion: " + factura.getSocio().getSucursal().getDireccion().getCalle() + " " + factura.getSocio().getSucursal().getDireccion().getAltura() + "-" + factura.getSocio().getSucursal().getDireccion().getLocalidad())
                .setTextAlignment(TextAlignment.CENTER));

        document.add(new Paragraph("DNI: "+factura.getSocio().getNumeroDocumento()+ "| IVA Responsable Inscripto")
                .setTextAlignment(TextAlignment.CENTER));

        document.add(new Paragraph("------------------------------------------------------------"));

        // ===== DATOS DE LA FACTURA =====
        document.add(new Paragraph("Factura N°: 0001-" + factura.getNumeroFactura()));
        document.add(new Paragraph("Fecha: " + factura.getFechaFactura().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"))));
        document.add(new Paragraph("Socio: " + factura.getSocio().getNombre() + " " + factura.getSocio().getApellido()));
        document.add(new Paragraph("N° Socio: " + factura.getSocio().getNumeroSocio()));
        document.add(new Paragraph("Forma de pago: " + factura.getFormaDePago().getNombre()));
        document.add(new Paragraph("Estado: " + factura.getEstado().toString()));
        document.add(new Paragraph(" "));

        // ===== TABLA DE DETALLES =====
        Table table = new Table(UnitValue.createPercentArray(new float[]{4, 2}));
        table.setWidth(UnitValue.createPercentValue(100));
        table.addHeaderCell(new Cell().add("Descripción").setBold().setBackgroundColor(ColorConstants.LIGHT_GRAY));
        table.addHeaderCell(new Cell().add("Monto ($)").setBold().setBackgroundColor(ColorConstants.LIGHT_GRAY));

        for (DetalleFactura detalle : factura.getDetalles()) {
            String descripcion = "Cuota mensual - " +
                    detalle.getCuotaMensual().getMes() + "/" +
                    detalle.getCuotaMensual().getAnio();

            table.addCell(new Cell().add(descripcion));
            table.addCell(new Cell().add(detalle.getMonto().toPlainString()));
        }
        ...
        // ===== TOTAL =====
        document.add(new Paragraph(" "));
        document.add(new Paragraph("TOTAL PAGADO: $ " + factura.getTotalPagado())
                .setBold()
