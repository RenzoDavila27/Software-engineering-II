...
public class FacturaController {
    ...
    public String listaFactura(Model model, @AuthenticationPrincipal Usuario usuario) {
        
        // El filtro JWT y SecurityConfig garantizan que 'usuario' no es null.
        // Obtenemos el rol directamente del objeto Usuario autenticado.
        if (usuario.getRol() == RolUsuario.SOCIO) {
            List<Factura> lista = facturaService.listarFacturaPorUsuario(usuario.getId());
            model.addAttribute("listaFactura", lista);
        } else {
            // Lógica para EMPLEADO y ADMINISTRADOR
